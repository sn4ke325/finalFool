                      ...
                      arg.add($e2.ast);
                     })*)? RPAR 
                                {
                                 ...
                                 if (vtable.containsKey($i2.text)) {
                                 	methodentry = vtable.get($i2.text); //è una STentry
                                 	if (!methodentry.isMethod()) {//se ho trovato qualcosa ma non è un metodo
                                 		System.out.println("Method " + $i2.text + "at line " + $i2.line + " is not declared for class "
                                 				+ ((ClassTypeNode) entry.getType()).getId());
                                 		System.exit(0);
                                 	}
                                 } else {
                                 	System.out.println("Method " + $i2.text + "at line " + $i2.line + " is not declared for class "
                                 ...
                                 $ast = new ClassCallNode($i.text, $i2.text, entry, methodentry, arg, nestingLevel);
